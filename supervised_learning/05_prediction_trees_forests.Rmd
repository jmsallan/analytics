...
To implement random forests in prediction, we can use the `rf` package. The package implements classification or prediction depending of the type of the target variable: it applies classification if the target is a factor, and prediction if it is a numeric.
...
Let's train a regression tree with the `rpart` function using all features:
...
The results of the tuning are tht the best prediction is achieved with `cp=0.01`.
...
The results for the test set are much worse than for the train set, meaning that the model is overfitted to the train set. To illustrate this, let's see how has been the prediction in the train test:
