...
There does not seem to be differences of the range of ages of the two groups.
...
When we look for between jobs differences, we can see that students and retired customers tend to suscribe more deposits.
...
Divorced and single people tend to suscribe more deposits than married customers.
...
Again, there does not seem to be significant differences between outcome-based groups in customer-related variables.
...
The type of contact does not influence the outcome, as proportions between groups are similar. As for number of contacts, the mean number of contacts between groups is similar:
...
As for previous contacts,There are two groups in the sample:
...
We define a new variable to label previously contacted individuals. Note that we are modifying `banks`, so we need to overwrite the data frame:
...
The strongest predictor of success in the campaign of suscription of new term deposits is that the customer has been sucessfully contacted in previous campaigns. To a lesser extent, the variables job, marital status and educational level present differences between sucessful and unsucessful contacts. People with no personal or housing loans tend to suscribe term deposits. As for the marketing campaign, duration of call is a predictor of interest in suscribing a term deposit.

As a conclusion, we can recommend that the banks builds a dataset of people sucessfully contacted in marketing campaigns, as a potential target for new campaigns. We also recommend re-contacting customers that have made a long contact in the present campaign.
